...
typedef struct {
  double cx, cy, cz; /* center of surface */
...
void makbox(int surf, double x, double y, double z, double xs, double yx,
    double zs);
...
void maksup(int surf, double cx, double cy, double cz, double A, double B,
    ...
    double J, double zmin, double zmax);
...
double (*objint[5])(t_3d *pos, t_3d *ray,
    t_object *obj) = {intsph,intbox,inttri,
      intsup,intplane };
...
void (*objpat[5])(t_3d pt, t_object *obj, double *u,
    double *v) = {patsph,patbox,
...
double max_intensity = 300.0; /* should be calculated in pgm, not here */
...
void setup() {
  ...
  sizex = SCREENWIDTH; /* screen dimensions */
  sizey = SCREENHEIGHT;
  ...
  maksup(5, 20.0, 50.0, 0.0, 1. / 100., 0., 0., 0., 1. / 100., 0., 0.,
      -1. / (800.), 0., 0., 0.0, 70.0);
  ...
  //max_intensity = 0.0; /* This should be calculated by the program */
}
...
void startpic(char outfile[60], int y, int x) {
  ...
  png_set_IHDR(png_ptr, info_ptr, SCREENWIDTH, SCREENHEIGHT, depth,
...
int calc_color_index(double intensity) {
  ...
  dval *= 255.0;  // maximum value for 8bits
  ival = (int) (dval + 0.5);
  return (ival);
}
...
double intersect(int source, t_3d *pos, t_3d *ray, t_color* color) {
  ...
  for (objtry = 0; objtry < nobject; objtry++) {
    ...
    {
      s = (*objint[object[objtry].objtyp])(pos, ray, &object[objtry]);
  ...
  /* find normal */
  (*objnrm[object[objhit].objtyp])(&hit, &object[objhit], &normal);
...
double brightness(int source, int lnum, t_3d *pos, t_3d *ray) {
  ...
  for (i = 0; i < nobject; i++) {
    ...
    {
      s = (*objint[object[i].objtyp])(pos, ray, &object[i]);
      /* keep track of closest intersection */
      if ((s > 0.0) && (s < ss))
        ss = s;
    }
...
double intsph(t_3d *pos, t_3d *ray, t_object *obj) {
  ...
  C1 = squared(radius) - squared(center.x) - squared(center.y)
      - squared(center.z);
...
void makbox(int surf, double x, double y, double z, double xs, double ys,
    double zs) {
  o_box *box;
...
int inside(t_3d point, o_triangle *triangle) {
  if (dotp(&point, &(triangle->e1)) < triangle->d1)
    return 0;
  if (dotp(&point, &(triangle->e2)) < triangle->d2)
    return 0;
  if (dotp(&point, &(triangle->e3)) < triangle->d3)
    return 0;
  return 1;
}
...
double intsup(t_3d *pos, t_3d *ray, t_object *obj) {
  ...
  K1 = -A * cx * cx - 2. * B * cx * cy - 2. * C * cx * cz + 2. * D * cx
      - E * cy * cy - 2. * F * cy * cz - H * cz * cz + 2. * I * cz - J;
  K2 = 2. * A * cx + 2. * B * cy + 2. * C * cz - 2. * D;
  K3 = 2. * B * cx + 2. * E * cy + 2. * F * cz - 2. * G;
  K4 = 2. * C * cx + 2. * F * cy + 2. * H * cz - 2. * I;

  K5 = K1 - A * x0 * x0 + K2 * x0 - E * y0 * y0 + K3 * y0 - H * z0 * z0
      + K4 * z0 - 2. * B * x0 * y0 - 2. * C * x0 * z0 - 2. * F * y0 * z0;
  K6 = A * xd * xd + E * yd * yd + H * zd * zd + 2. * B * xd * yd
      + 2. * C * xd * zd + 2. * F * yd * zd;
  K7 = 2. * A * x0 * xd - K2 * xd + 2 * E * y0 * yd - K3 * yd + 2. * H * z0 * zd
      - K4 * zd + 2. * B * x0 * yd + 2. * B * y0 * xd + 2. * C * x0 * zd
      + 2. * C * z0 * zd + 2. * F * y0 * zd + 2. * F * z0 * yd;
  if (!solve_quad(K6, K7, -K5, &root1, &root2))
    return FAR_AWAY;
  h1 = pos->z + root1 * ray->z;
  h2 = pos->z + root2 * ray->z;
  if ((h1 > superq->zmax) || (h1 < superq->zmin))
    root1 = 0.0;
  if ((h2 > superq->zmax) || (h2 < superq->zmin))
    root2 = 0.0;
  return min_pos(root1, root2);
}
...
void nrmsup(t_3d *pos, t_object *obj, t_3d *nrm) {
  ...
  y = pos->y - superq->cy;
  z = pos->z - superq->cz;
  nrm->x = 2. * superq->A * x + 2. * superq->B * y + 2. * superq->C * z
      + 2. * superq->D;
  nrm->y = 2. * superq->B * x + 2. * superq->E * y + 2. * superq->F * z
      + 2. * superq->G;
  nrm->z = 2. * superq->C * x + 2. * superq->F * y + 2. * superq->H * z
      + 2. * superq->I;
  normalize(nrm);
}
void patsph(t_3d pt, t_object *obj, double *u, double*v) {
  ...
  *v = (M_PI - phi) / M_PI;
  if (X / sin(phi) >= 1.0)
    theta = acos(1.0);
  else if (X / sin(phi) <= -1.0)
    theta = acos(-1.0);
  else
    theta = acos(X / sin(phi));
  if (Y > 0.)
    *u = theta / (2. * M_PI);
  else
    *u = 1. - (theta / (2. * M_PI));
